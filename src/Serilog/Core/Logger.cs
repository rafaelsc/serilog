...
namespace Serilog.Core
{
    ...
    public sealed class Logger : ILogger, ILogEventSink, IDisposable
    {
        ...
        public void Write<T>(LogEventLevel level, string messageTemplate, T propertyValue)
        {
            ...
            if (IsEnabledInternal(level))
            {
                WriteInternal(level, null, messageTemplate, new[] { propertyValue });
            }
        }
        ...
        public void Write<T>(LogEventLevel level, string messageTemplate, T propertyValue0, T propertyValue1)
        {
            ...
            if (IsEnabledInternal(level))
            {
                WriteInternal(level, null, messageTemplate, new[] { propertyValue0, propertyValue1 });
            }
        }
        ...
        public void Write<T>(LogEventLevel level, string messageTemplate, T propertyValue0, T propertyValue1, T propertyValue2)
        {
            ...
            if (IsEnabledInternal(level))
            {
                WriteInternal(level, null, messageTemplate, new[] { propertyValue0, propertyValue1, propertyValue2 });
            }
        }
        ...
        public void Write<T>(LogEventLevel level, Exception exception, string messageTemplate, T propertyValue)
        {
            ...
            if (IsEnabledInternal(level))
            {
                WriteInternal(level, exception, messageTemplate, new[] { propertyValue });
            }
        }
        ...
        public void Write<T>(LogEventLevel level, Exception exception, string messageTemplate, T propertyValue0, T propertyValue1)
        {
            ...
            if (IsEnabledInternal(level))
            {
                WriteInternal(level, exception, messageTemplate, new[] { propertyValue0, propertyValue1 });
            }
        }
        ...
        public void Write<T>(LogEventLevel level, Exception exception, string messageTemplate, T propertyValue0, T propertyValue1, T propertyValue2)
        {
            ...
            if (IsEnabledInternal(level))
            {
                WriteInternal(level, exception, messageTemplate, new[] { propertyValue0, propertyValue1, propertyValue2 });
            }
        }
        ...
        {
            WriteInternal<object>(level, null, messageTemplate, NoPropertyValues);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        void WriteInternal(LogEventLevel level, Exception exception, string messageTemplate)
        {
            WriteInternal<object>(level, exception, messageTemplate, NoPropertyValues);
        }
        
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        void WriteInternal(LogEventLevel level, Exception exception, string messageTemplate, object[] propertyValues)
        {
            // Avoid the array allocation and any boxing allocations when the level isn't enabled or a null messageTemplate
            ...
            WriteInternal<object>(level, exception, messageTemplate, propertyValues);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        void WriteInternal<T>(LogEventLevel level, Exception exception, string messageTemplate, T[] propertyValues)
        {
            // Avoid the Processor run when the level isn't enabled or a null messageTemplate
            if (!IsEnabledInternal(level)) return;
            if (messageTemplate == null) return;
